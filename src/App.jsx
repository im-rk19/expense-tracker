...
import { ChevronLeft, ChevronRight, Plus, X, IndianRupee, Calendar as CalendarIcon, LogOut, Download, Upload, Trash2, PieChart, Filter } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-black p-6 animate-in fade-in duration-700">
      <div className="w-full max-w-sm text-center">
        ...
        <p className="mt-12 text-slate-700 text-[9px] font-bold uppercase tracking-[0.2em]">100% Local • 0% Cloud • Private</p>
      </div>
    </div>
  );
};
...
const Header = ({ isInstalled, handleInstallClick, handleLogout, onExport, onImport }) => (
  <header className="flex justify-between items-start mb-8 pt-4">
    <div className="flex items-center gap-3">
      <div className="bg-red-600/10 border border-red-600/20 p-2 rounded-xl shadow-[0_0_15px_rgba(220,38,38,0.1)]">
        <IndianRupee className="text-red-500" size={20} strokeWidth={3} />
      </div>
      <div>
        <h1 className="text-xl font-black text-white tracking-tighter uppercase leading-none">Expense Tracker</h1>
        <p className="text-[9px] font-bold text-red-500/80 uppercase tracking-widest mt-1">Local Ledger Only</p>
      </div>
    </div>
    <div className="flex items-center gap-2">
       {!isInstalled && (
         <button onClick={handleInstallClick} className="text-[9px] font-black px-3 py-1.5 rounded-full border border-red-600/30 text-red-500 hover:bg-red-600 hover:text-white transition-all uppercase">
           Install
         </button>
       )}
       <button onClick={onImport} className="p-2 text-slate-500 hover:text-white transition-colors"><Upload size={16}/></button>
       <button onClick={onExport} className="p-2 text-slate-500 hover:text-white transition-colors"><Download size={16}/></button>
       <button onClick={handleLogout} className="p-2 text-slate-500 hover:text-red-500 transition-colors"><LogOut size={16}/></button>
    </div>
  </header>
);
...
const MonthBar = ({ currentMonth, setCurrentMonth }) => {
  ...
  return (
    <div ref={scrollRef} className="flex gap-2 overflow-x-auto no-scrollbar pb-2 mb-4 mask-fade-edges">
      {months.map((m, i) => (
        <button 
          key={m} 
          onClick={() => setCurrentMonth(new Date(currentMonth.getFullYear(), i, 1))}
          className={`px-6 py-2 rounded-xl text-[10px] font-black transition-all flex-shrink-0 border ${currentMonth.getMonth() === i ? 'bg-red-600 border-red-400 text-white shadow-lg shadow-red-600/20' : 'bg-slate-900 border-slate-800 text-slate-500'}`}
        >
          {m}
        </button>
      ))}
...
const ExpenseTracker = () => {
  ...
  const saveExpenses = (newExpenses) => {
    ...
    localStorage.setItem('local_expenses', JSON.stringify(newExpenses));
  };
  ...
  const handleExport = () => {
    ...
    const csvContent = [
      ...
      ...expenses.map(e => {
        ...
        return `"${e.date}",${e.amount},"${cat}","${e.description.replace(/"/g, '""')}"`;
      })
    ].join("\n");
    ...
    input.onchange = (e) => {
      ...
      reader.onload = (event) => {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          const catName = parts[2].replace(/"/g, '');
          const desc = parts[3].replace(/"/g, '');
          ...
          newEntries.push({
            ...
            amount,
            description: desc,
            categoryId: DEFAULT_CATEGORIES.find(c => c.name === catName)?.id || 10
          });
        }
        if (newEntries.length > 0) {
          saveExpenses([...newEntries, ...expenses]);
  ...
  const monthlyTotal = filteredExpenses.reduce((s, e) => s + e.amount, 0).toFixed(2);
  ...
  return (
    <div className="min-h-screen bg-black text-white p-4 font-sans pb-32">
      <div className="max-w-md mx-auto">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 text-white shadow-2xl relative overflow-hidden mb-6 group transition-all hover:border-red-600/50">
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="grid grid-cols-2 gap-3">
              {DEFAULT_CATEGORIES.slice(0, 4).map(cat => {
                const catTotal = filteredExpenses.filter(e => e.categoryId === cat.id).reduce((s, e) => s + e.amount, 0).toFixed(0);
                return (
            ...
            <div className="space-y-3">
              <p className="text-[10px] font-black text-slate-600 uppercase tracking-[0.2em] px-2">Transactions</p>
              <div className="space-y-2">
                {filteredExpenses.slice(0, 20).map(expense => {
                   const cat = DEFAULT_CATEGORIES.find(c => c.id === expense.categoryId);
                   return (
                     <div key={expense.id} className="bg-slate-900/40 border border-slate-800 rounded-2xl p-4 flex justify-between items-center group active:scale-[0.98] transition-all">
                       <div className="flex items-center gap-4">
                         <div className="text-2xl">{cat?.emoji}</div>
                         <div>
                           <p className="text-sm font-bold text-white leading-none mb-1">{expense.description}</p>
                           <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest">{new Date(expense.date).toLocaleDateString('en-IN', { day: '2-digit', month: 'short' })}</p>
                         </div>
                       </div>
                       <div className="flex items-center gap-4">
                         <p className="text-lg font-black text-white">₹{expense.amount.toFixed(0)}</p>
                ...
                {filteredExpenses.length === 0 && (
                  <div className="py-20 text-center space-y-3 opacity-30">
                    <PieChart size={48} className="mx-auto text-slate-700" />
                    <p className="text-[10px] font-black uppercase tracking-widest">No entries for this month</p>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="animate-in fade-in duration-500 space-y-6">
                  ...
                  {Array.from({ length: new Date(currentMonth.getFullYear(), currentMonth.getMonth() + 1, 0).getDate() }, (_, i) => i + 1).map(day => {
                    ...
                    return (
                      <button key={day} onClick={() => setSelectedDate(dateStr)} 
                        ...
                        {day}
                        {hasExpense && <div className="absolute bottom-1 w-1 h-1 bg-red-400 rounded-full" />}
                      </button>
                    );
                  })}
               </div>
            ...
            <div className="space-y-3">
              ...
              {expenses.filter(e => e.date === selectedDate).map(e => (
                <div key={e.id} className="bg-slate-900/50 border border-slate-800 p-4 rounded-2xl flex justify-between items-center animate-in slide-in-from-left-4">
                  <div className="flex items-center gap-3">
        ...
        {view === 'add' && (
          <div className="animate-in zoom-in-95 duration-300">
            <div className="bg-slate-900 border border-slate-800 rounded-[2.5rem] p-8 shadow-2xl">
              <h2 className="text-white font-black mb-8 uppercase tracking-[0.2em] text-[10px] text-center text-red-500">Record Transaction</h2>
              <div className="space-y-6">
                ...
                <div className="space-y-1">
                  ...
                  <div className="relative">
                    ...
                    <input type="number" value={amount} onChange={e => setAmount(e.target.value)} placeholder="0.00" className="w-full bg-black border border-slate-800 rounded-2xl pl-12 pr-5 py-5 text-white text-3xl font-black outline-none focus:border-red-600 transition-colors" />
                  </div>
                </div>

                <div className="space-y-1">
                  <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest ml-2">Remark</p>
                  <input type="text" value={description} onChange={e => setDescription(e.target.value)} placeholder="Spent on..." className="w-full bg-black border border-slate-800 rounded-2xl px-5 py-4 text-white text-sm outline-none focus:border-red-600 transition-colors" />
                </div>

                <div className="flex gap-3 pt-4">
                  <button onClick={handleAddExpense} disabled={!selectedCategory || !amount} className="flex-[2] bg-red-600 text-white py-5 rounded-[1.5rem] font-black shadow-xl disabled:opacity-30 active:scale-95 transition-all uppercase tracking-widest">Save</button>
                  <button onClick={() => setView('overview')} className="flex-1 bg-slate-800 text-white py-5 rounded-[1.5rem] font-black active:scale-95 transition-all uppercase tracking-widest text-[10px]">Cancel</button>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="fixed bottom-8 left-0 right-0 px-8 z-50">
          <div className="max-w-md mx-auto bg-slate-900/90 backdrop-blur-xl border border-slate-800/50 p-2 rounded-[2rem] shadow-2xl flex gap-2">
            <button onClick={() => setView('overview')} className={`flex-1 py-4 rounded-2xl font-black flex items-center justify-center gap-2 transition-all ${view === 'overview' ? 'bg-red-600 text-white shadow-lg shadow-red-600/20' : 'text-slate-500'}`}>
              <TrendingUp size={20} />
            </button>
            ...
            <button onClick={() => setView('calendar')} className={`flex-1 py-4 rounded-2xl font-black flex items-center justify-center gap-2 transition-all ${view === 'calendar' ? 'bg-red-600 text-white shadow-lg shadow-red-600/20' : 'text-slate-500'}`}>
